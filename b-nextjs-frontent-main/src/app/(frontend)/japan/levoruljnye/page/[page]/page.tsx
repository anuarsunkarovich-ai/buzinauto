...
import { HOME_BREADCRUMB, JAPAN_CAR_LEVORULJNYE, JAPAN_CAR_ROOT } from '@/constants/breadcrumb'
...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.middleJapan('леворульные авто', 'Заказать') + generatePage.end(page),
    title: generateTitle.middleJapan('леворульные авто', 'Купить') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function JapanCarsPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  return (
    ...
    >
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, JAPAN_CAR_ROOT, JAPAN_CAR_LEVORULJNYE]} />
        <Title as="h1">
          {generateH1.endJapan('', 'Леворульные автомобили') + generatePage.end(page)}
        </Title>
        <FilterAuto />
        <CarCarouselOnHover items={items} />
        <PayloadPaginationAuto path="/japan/levoruljnye" {...pagination} />
      </BoxContainer>
      <Footer />
    </div>
  )
