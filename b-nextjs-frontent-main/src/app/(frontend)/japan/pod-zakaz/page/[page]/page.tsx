...
import { HOME_BREADCRUMB, JAPAN_CAR_POD_ZAKAZ, JAPAN_CAR_ROOT } from '@/constants/breadcrumb'
...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.endJapan('под заказ', 'Авто') + generatePage.end(page),
    title: generateTitle.endJapan('под заказ', 'Авто') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function JapanCarsPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  return (
    ...
    `}>
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, JAPAN_CAR_ROOT, JAPAN_CAR_POD_ZAKAZ]} />
        <Title as="h1">{generateH1.endJapan('под заказ', 'Авто') + generatePage.end(page)}</Title>
        <FilterAuto />
        <CarCarouselOnHover items={items} />
        <PayloadPaginationAuto path="/japan/pod-zakaz" {...pagination} />
      </BoxContainer>
      <Footer />
    </div>
  )
