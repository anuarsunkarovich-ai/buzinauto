...
import { HOME_BREADCRUMB, JAPAN_CAR_PRAVORULJNYE, JAPAN_CAR_ROOT } from '@/constants/breadcrumb'
...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description:
      generateTitle.middleJapan('праворульные авто', 'Заказать') + generatePage.end(page),
    title: generateTitle.middleJapan('праворульные авто', 'Купить') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function JapanCarsPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  return (
    ...
    >
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, JAPAN_CAR_ROOT, JAPAN_CAR_PRAVORULJNYE]} />
        <Title as="h1">
          {generateH1.endJapan('', 'Праворульные автомобили') + generatePage.end(page)}
        </Title>
        <FilterAuto />
        <CarCarouselOnHover items={items} />
        <PayloadPaginationAuto path="/japan/pravoruljnye" {...pagination} />
      </BoxContainer>
      <Footer />
    </div>
  )
