...
import { HOME_BREADCRUMB, JAPAN_CAR_ROOT, JAPAN_CAR_S_PROBEGOM } from '@/constants/breadcrumb'
...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.endJapan('с пробегом') + generatePage.end(page),
    title: generateTitle.endJapan('с пробегом') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function JapanCarsPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  return (
    ...
    `}>
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, JAPAN_CAR_ROOT, JAPAN_CAR_S_PROBEGOM]} />
        <Title as="h1">{generateH1.endJapan('с пробегом') + generatePage.end(page)}</Title>
        <FilterAuto />
        <CarCarouselOnHover items={items} />
        <PayloadPaginationAuto path="/japan/s-probegom" {...pagination} />
      </BoxContainer>
      <Footer />
    </div>
  )
