...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description:
      generateTitle.middleAuctionChina(toReadableSlug(brand), 'Заказать') + generatePage.end(page),
    title:
      generateTitle.middleAuctionChina(toReadableSlug(brand), 'Купить') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
  ...
  return (
    ...
    >
      ...
      <BoxContainer>
        ...
        />
        <Title as="h1">
          {generateH1.middleAuctionChina('', toReadableSlug(brand)) + generatePage.end(page)}
        </Title>
        <FilterAuto />
