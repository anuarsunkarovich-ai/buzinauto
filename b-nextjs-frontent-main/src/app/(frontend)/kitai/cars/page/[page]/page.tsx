...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.endAuctionChina('авто', 'Каталог') + generatePage.end(page),
    title: generateTitle.endAuctionChina('авто', 'Каталог') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function ChinaCarsPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  return (
    ...
    `}>
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, CHINA_BREADCRUMB, CHINA_CATALOG_BREADCRUMB]} />
        <Title as="h1">
          {generateH1.middleAuctionChina('автомобилей', 'Каталог') + generatePage.end(page)}
        </Title>
        <FilterAuto />
