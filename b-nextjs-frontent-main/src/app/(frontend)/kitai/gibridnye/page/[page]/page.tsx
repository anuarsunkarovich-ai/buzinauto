...
import { CHINA_BREADCRUMB, CHINA_CAR_HYBRID, HOME_BREADCRUMB } from '@/constants/breadcrumb'
...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.middleChina('гибридные авто', 'Заказать') + generatePage.end(page),
    title: generateTitle.middleChina('гибридные авто', 'Купить') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function ChinaCarsPage({ params }: { params: Promise<{ page: string }> }) {
  ...
  const { docs, ...pagination } = await getManyCatalogCarPromise(Number(page), {
    ...
    engineTypes: ['hybrid-diesel', 'hybrid-gasoline'],
  })
  ...
  return (
    ...
    >
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, CHINA_BREADCRUMB, CHINA_CAR_HYBRID]} />
        <Title as="h1">
          {generateH1.middleChina('автомобили', 'Гибридные') + generatePage.end(page)}
        </Title>
        <FilterAuto />
        <CarCarouselOnHover items={items} />
        <PayloadPaginationAuto path="/kitai/gibridnye" {...pagination} />
      </BoxContainer>
      <Footer />
    </div>
  )
