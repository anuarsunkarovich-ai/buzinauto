...
import { CHINA_BREADCRUMB, CHINA_CAR_NEW, HOME_BREADCRUMB } from '@/constants/breadcrumb'
...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.middleChina('новый авто', 'Заказать') + generatePage.end(page),
    title: generateTitle.middleChina('новый авто', 'Купить') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
...
export default async function ChinaCarsPage({ params }: Params) {
  ...
  return (
    ...
    >
      <Header />
      <BoxContainer>
        <AppBreadcrumb items={[HOME_BREADCRUMB, CHINA_BREADCRUMB, CHINA_CAR_NEW]} />
        <Title as="h1">
          {generateH1.middleChina('автомобили', 'Новые') + generatePage.end(page)}
        </Title>
        <FilterAuto />
        <CarCarouselOnHover items={items} />
        <PayloadPaginationAuto path="/kitai/novyi" {...pagination} />
      </BoxContainer>
      <Footer />
    </div>
  )
