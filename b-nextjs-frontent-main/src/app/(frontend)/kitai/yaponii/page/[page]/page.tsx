...
export async function generateMetadata({ params: paramsPromise }: Params): Promise<Metadata> {
  ...
  return {
    description: generateTitle.middleChina('Японские авто', 'Заказать') + generatePage.end(page),
    title: generateTitle.middleChina('Японские авто', 'Купить') + generatePage.end(page),
    alternates: {
      canonical: './',
    },
  }
  ...
  return (
    ...
    >
      ...
      <BoxContainer>
        ...
        <Title as="h1">
          {generateH1.middleChina('автомобили', 'Японские') + generatePage.end(page)}
        </Title>
        <FilterAuto />
